...
function EditProduct() {
    ...
    const { getOneProductDetails, productDetails, updateProduct } =
        useProducts();
    const { id } = useParams();
    ...
    const [uploadProgressEdit, setUploadProgressEdit] = useState(null);
    ...
    const handleInp = (e) => {
        let obj = { ...editedProduct, [e.target.name]: e.target.value }; // берет все что было в инпуте, если мы что-то меняем то пусть поменяет
        setEditedProduct(obj); // и тут обновляем и отправляем те данные
    };
